...
class ChatOrchestrator:
    ...
    def __init__(
        ...
        Args:
            ...
            rate_limiter: Optional rate limiter (uses config defaults if None)
        """
    ...
    async def send_message(
        ...
        sanitized = self._sanitizer.sanitize_with_metadata(content)
        ...
        truncated = self._tokenizer.truncate_context(
            history, conv.model_id, max_context
        )
        ...
        api_messages = [
            {"role": msg.role.value, "content": msg.content} for msg in truncated
        ]
    ...
    def _calculate_cost(self, usage: TokenUsage, model: OpenRouterModel) -> Decimal:
        ...
        return usage.calculate_cost(
            model.prompt_price or Decimal("0"), model.completion_price or Decimal("0")
        )
    ...
    def export_conversation(self, conversation_id: UUID, format: str = "json") -> str:
        """Export a conversation to JSON or Markdown format.

        Phase 6: Export Functionality - Export conversations for backup/sharing.
